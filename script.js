  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const toggleDownloadSelectedButton = () => {
      downloadSelectedButton.style.display = pdfInput.files.length && pageInput.value.trim() ? 'inline-block' : 'none';
    };
    ...
    const renderPage = async (pageNumber) => {
      ...
      const pageInfoText = totalPages === 1 ? 'Page' : 'Pages';
      pdfPageInfo.textContent = `${pageNumber} ${pageInfoText} ${totalPages}`;
    ...
    pdfInput.addEventListener('change', async (e) => {
      ...
      statusMessage.textContent = `Converting 1 of ${pdf.numPages}`;
      scrollToStatus();
    ...
    downloadAllButton.addEventListener('click', async () => {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Update status
        statusMessage.textContent = `Converting ${i} of ${pdf.numPages}`;
        scrollToStatus();
      }

      statusMessage.textContent = 'Archiving';
      scrollToStatus();

      zip.generateAsync({ type: 'blob' }, function updateCallback(metadata) {
        const percent = metadata.percent.toFixed(2);
        statusMessage.textContent = `Archiving ${percent}%`;
        scrollToStatus();
      })
      .then((blob) => {
      ...
      for (const pageRange of pageRanges) {
        ...
        if (rangeParts.length === 1) {
          ...
          if (!isNaN(pageNumber) && pageNumber >= 1 && pageNumber <= totalPageCount) {
            ...
            processedPages++;
            // Update status
            statusMessage.textContent = `Converting ${processedPages} of ${totalPagesToConvert}`;
            scrollToStatus();
          }
        } else if (rangeParts.length === 2) {
          const startPage = parseInt(rangeParts[0]);
    ...
    pdfInput.addEventListener('change', function() {
      const isFileSelected = !!this.files.length;
      ...
      document.getElementById('downloadAll').style.display = isFileSelected ? 'inline-block' : 'none';
